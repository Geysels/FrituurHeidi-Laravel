@if (Session::has('cart'))
    <ul>
        @foreach ($products as $product)
            <li><span
                    class="mr-2 rounded bg-pink-100 px-2.5 py-0.5 text-sm font-medium text-pink-800 dark:bg-pink-200 dark:text-pink-900">{{ $product['qty'] }}</span>
                <p>{{ $product['item']['name'] }}</p>
                <span
                    class="mr-2 rounded bg-pink-100 px-2.5 py-0.5 text-sm font-medium text-pink-800 dark:bg-pink-200 dark:text-pink-900">{{ $product['price'] }}</span>
